...
# Silent README Analytics Update
# Runs quietly and only updates if there are changes

TEMP_README="README_SILENT_TEMP.md"
...
TOTAL_AUTHORS=$(git log --pretty=format:"%an" 2>/dev/null | sort | uniq | wc -l)
...
echo "\`\`\`" >> "$TEMP_README"
git log --oneline -8 2>/dev/null | sed 's/^/🔹 /' >> "$TEMP_README"
echo "\`\`\`" >> "$TEMP_README"
...
echo "\`\`\`" >> "$TEMP_README"
git log --pretty=format: --name-only 2>/dev/null | sort | uniq -c | sort -rn | head -5 | sed 's/^/📄 /' >> "$TEMP_README"
echo "\`\`\`" >> "$TEMP_README"
...
# Update README.md silently
if [ -f "README.md" ]; then
    # Remove old analytics section if it exists
    awk "/$ANALYTICS_SECTION/,/<!-- END_ANALYTICS -->/ { next } { print }" README.md > README_temp.md 2>/dev/null
...
$ANALYTICS_SECTION\\
$(cat $TEMP_README)\\
<!-- END_ANALYTICS -->" README_temp.md > README.md 2>/dev/null
    else
